...
Item {
    ...
    property real kbdBrightness: widgetCache.kbdBrightness
            ...
            onStreamFinished: {
                ...
                if (lines.length >= 5) {
                    ...
                    if (!window.isDraggingKbd) {
                        window.kbdBrightness = parseInt(lines[5]) || 0;
                        widgetCache.kbdBrightness = window.kbdBrightness;
                    }
                }
            }
        }
            ...
            RowLayout {
                ...
                Quickshell.execDetached(["sh", "-c", "echo 'close' > " + paths.runDir + "/widget_state"]);
                    ...
                    Grid {
                        ...
                        // 1. CPU Orb
                        SysDataOrb {
                            animVal: window.cpuUsage
                            valueText: Math.round(animVal) + "%"
                            labelText: "CPU LOAD"
                            iconText: ""
                            colorMain: window.blue
                            colorSecondary: window.sapphire
                        }

                        // 2. RAM Orb
                        SysDataOrb {
                            animVal: window.ramUsage
                            valueText: Math.round(animVal) + "%"
                            labelText: "MEMORY"
                            iconText: "󰍛"
                            colorMain: window.mauve
                            colorSecondary: window.pink
                        }

                        // 3. DISK Orb
                        SysDataOrb {
                            animVal: window.diskUsage
                            valueText: Math.round(animVal) + "%"
                            labelText: "STORAGE"
                            iconText: "󰋊"
                            colorMain: window.peach
                            colorSecondary: window.yellow
                        }

                        // 4. TEMP Orb
                        SysDataOrb {
                            animVal: window.sysTemp
                            valueText: Math.round(animVal) + "°"
                            labelText: "SYSTEM TEMP"
                            iconText: ""
                            colorMain: window.red
                            colorSecondary: window.maroon
                        }
                    }
