...
Item {
    ...
    property bool isDraggingKbd: false
    ...
    Timer { id: kbdSyncDelay; interval: 800; onTriggered: window.isDraggingKbd = false; triggeredOnStart: true; }
    ...
    Process {
        ...
        command: ["bash", "-c", 
            ...
            "bash ~/.config/hypr/scripts/get_brightness.sh; " +
            "brightnessctl -d '*kbd_backlight*' -m 2>/dev/null | awk -F, '{print substr($4, 1, length($4)-1)}' || echo '0'"
        ]
        running: true
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                if (lines.length >= 6) {
                    ...
                    if (!window.isDraggingKbd) {
                        window.kbdBrightness = parseInt(lines[6]) || 0;
                    }
                }
            }
        }
    ...
    Timer {
        interval: window.visible ? 1500 : 8000; running: true; repeat: true; triggeredOnStart: true;
    ...
    Timer {
        ...
        repeat: true
        onTriggered: {
            window.globalOrbitAngle = (window.globalOrbitAngle + (Math.PI * 2 / (90000 / 33))) % (Math.PI * 2)
        }
    }
                ...
                Item {
                    ...
                    Row {
                        ...
                        Text {
                            ...
                            SequentialAnimation on uptimePulse {
                                loops: Animation.Infinite; running: window.opacity > 0
                    ...
                    Item {
                        ...
                        Rectangle {
                            ...
                            width: centralCore.width + window.s(35)
                            height: centralCore.height + window.s(35)
                            radius: width / 2
                        ...
                        Rectangle {
                            ...
                            Item {
                                ...
                                Canvas {
                                    ...
                                    Timer {
                                        running: (heroMa.containsMouse || window.isCharging) && window.opacity > 0.0
                                        repeat: true
                                        interval: 33 
                                    ...
                                    onPaint: {
                                        ...
                                        var centerX = width / 2;
                                        var tubeRadius = (width / 2) - window.s(12);
                                        var topY = tubeRadius + window.s(12);
                                        var bottomY = height - tubeRadius - window.s(12);
                                        
                                        ctx.lineCap = "round";
                                        
                                        // Draw tube background outline
                                        ctx.lineWidth = window.s(8);
                                        ctx.beginPath();
                                        ctx.arc(centerX, topY, tubeRadius, Math.PI, 0);
                                        ctx.lineTo(centerX + tubeRadius, bottomY);
                                        ctx.arc(centerX, bottomY, tubeRadius, 0, Math.PI);
                                        ctx.closePath();
                                        ctx.strokeStyle = window.surface1;
                                        ctx.stroke();
                                        
                                        var batColor1 = window.isCharging ? "#39ff14" : window.batColorStart.toString();
                                        var batColor2 = window.isCharging ? "#39ff14" : window.batColorEnd.toString();
                                        
                                        var fillGrad = ctx.createLinearGradient(0, height, width, 0);
                                        fillGrad.addColorStop(0, batColor1);
                                        fillGrad.addColorStop(1, batColor2);
                                        ...
                                        ctx.arc(centerX, bottomY, tubeRadius, 0, Math.PI);
                                        ctx.closePath();
                                        ctx.clip(); // Clip to the tube

                                        var fillPercent = window.animCapacity / 100.0;
                                        var liquidY = height - (height * fillPercent);
                                        
                                        ctx.beginPath();
                                        var waveAmplitude = (heroMa.containsMouse || window.isCharging) ? window.s(12) : window.s(4);
                                        var waveOffset = (Date.now() / 250) % (Math.PI * 2);
                                        
                                        ctx.moveTo(0, height);
                                        ctx.lineTo(0, liquidY);
                                        for (var x = 0; x <= width; x += 5) {
                                            var y = liquidY + Math.sin((x / 25) + waveOffset) * waveAmplitude;
                                            ctx.lineTo(x, y);
                                        }
                                        ...
                                        if (window.isCharging) {
                                            ...
                                            for(var b=0; b<bCount; b++) {
                                                var bubblePhase = (Date.now() / (1000 + b*200) + (b*0.3)) % 1.0;
                                                if (height - (bubblePhase * height) > liquidY) {
                                                    ctx.beginPath();
                                                    ctx.arc(centerX - window.s(30) + (b*15), height - (bubblePhase * height), window.s(3 + b%4), 0, 2*Math.PI);
                                                    ctx.globalAlpha = 0.6 * (1.0 - bubblePhase);
                                                    ctx.fill();
                                                }
                                            }
                                        }
                        ...
                        ColumnLayout {
                                    ...
                                    RowLayout {
                                        ...
                                        Item {
                                            ...
                                            Timer {
                                                ...
                                                onTriggered: {
                                                    if (targetPct >= 0) {
                                                        Quickshell.execDetached(["bash", "-c", "~/.config/hypr/scripts/set_brightness.sh " + targetPct]);
                                                        targetPct = -1;
                                                    }
                                                }
                                            }
                                    ...
                                    RowLayout {
                                        ...
                                        Item {
                                            ...
                                            Timer {
                                                ...
                                                onTriggered: {
                                                    if (targetPct >= 0) {
                                                        Quickshell.execDetached(["brightnessctl", "-d", "*kbd_backlight*", "set", targetPct + "%"]);
                                                        targetPct = -1;
                                                    }
                                                }
                                            }
                                            ...
                                            Rectangle {
                                                ...
                                                Rectangle {
                                                    height: parent.height
                                                    width: parent.width * (window.kbdBrightness / 100)
                                                    radius: window.s(9)
                                                    opacity: kbdMa.containsMouse ? 1.0 : 0.85
                                                    Behavior on opacity { NumberAnimation { duration: 200 } }
                                                    Behavior on width { enabled: !window.isDraggingKbd; NumberAnimation { duration: 200; easing.type: Easing.OutQuint } }
                                            ...
                                            MouseArea {
                                                ...
                                                function updateKbd(mx) {
                                                    let pct = Math.max(0, Math.min(100, Math.round((mx / width) * 100)));
                                                    window.kbdBrightness = pct; 
                                                    kbdCmdThrottle.targetPct = pct;
                                                    if (!kbdCmdThrottle.running) kbdCmdThrottle.start();
                                                }
                                            }
                                        }
                                    }
                            ...
                            Rectangle {
                                ...
                                Rectangle {
                                    ...
                                    color: Qt.rgba(window.profileStart.r, window.profileStart.g, window.profileStart.b, 0.4)
                                    border.color: window.profileStart
                                    border.width: 1
                                    Behavior on color { ColorAnimation { duration: 400 } }
                                    ...
                                    layer.effect: MultiEffect {
                                        shadowEnabled: true
                                        shadowColor: window.profileStart
                                        shadowBlur: 0.4
