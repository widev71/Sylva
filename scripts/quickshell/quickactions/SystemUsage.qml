...
Item {
    ...
    Timer {
        interval: 500 // 2 updates per second
        running: root.widgetVisible
        repeat: true
        onTriggered: {
            root.globalWavePhase = (root.globalWavePhase + (Math.PI * 2 / (2400 / 500))) % (Math.PI * 2)
        }
    }
    ...
    component LiquidSquare: Item {
        ...
        Item {
            ...
            Text {
                ...
                font.family: "Inter"; font.bold: true; font.pixelSize: root.s(10)
            ...
            Text {
                ...
                font.family: "Inter"; font.bold: true; font.pixelSize: root.s(12)
                color: root.cSubtext0; text: ls.subText
            }
            Text {
                anchors.bottom: parent.bottom
                anchors.right: parent.right
                font.family: "Inter"; font.weight: Font.Black; font.pixelSize: root.s(24)
    ...
    component NeonRing: Item {
        ...
        Rectangle {
            ...
            radius: root.s(12)
            color: "transparent"
            border.color: root.alpha(root.cText, 0.05)
            border.width: 1
        }
        ...
        Canvas {
            ...
            onPaint: {
                ...
                var radius = Math.min(width, height) / 2 - root.s(15);
                var lineWidth = root.s(10);

                // Background Ring
                ctx.beginPath();
                ctx.arc(centerX, centerY, radius, 0, 2 * Math.PI);
                ctx.lineWidth = lineWidth;
                ctx.strokeStyle = root.alpha(ring.colorBase, 0.5);
                ctx.stroke();

                if (ring.value <= 0) return;

                // Foreground Ring (Value)
                var startAngle = -Math.PI / 2;
                var endAngle = startAngle + (ring.value * 2 * Math.PI);
                ...
                // Pulsing Effect
                var pulse = Math.sin(root.globalWavePhase * 2) * 0.5 + 0.5; // 0 to 1
                var activeColor = ring.value > 0.8 ? root.cRed : ring.colorFill;
                
                ctx.strokeStyle = activeColor;
                ctx.shadowColor = activeColor;
                ctx.shadowBlur = root.s(10) + (ring.value > 0.8 ? pulse * root.s(15) : 0);
            ...
            Connections {
                target: root
                enabled: root.widgetVisible && ring.value > 0
                function onGlobalWavePhaseChanged() { if (ring.value > 0.8) ringCanvas.requestPaint(); }
            }
        ...
        Item {
            ...
            Text {
                ...
                anchors.bottomMargin: root.s(4)
                font.family: root.iconFont; font.pixelSize: root.s(20)
                color: (ring.value > 0.8) ? root.cRed : ring.colorFill
                text: ring.icon
                style: Text.Outline
                styleColor: root.alpha(color, 0.4)
            }
            
            Text {
                id: valText
                anchors.centerIn: parent
                font.family: "Inter"; font.weight: Font.Black; font.pixelSize: root.s(18)
                color: root.cYellow
                text: ring.valueText
                style: Text.Outline
                styleColor: root.alpha(root.cYellow, 0.2)
            }
        ...
        LiquidSquare {
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    Text { text: root.rxSpeedStr; color: root.textPrimary; font.family: "Inter"; font.pixelSize: root.s(16); font.bold: true }
                }

                RowLayout {
                    spacing: root.s(12)
                    Text { text: "\uF062"; font.family: root.iconFont; font.pixelSize: root.s(16); color: root.cPeach }
                    Text { text: root.txSpeedStr; color: root.textPrimary; font.family: "Inter"; font.pixelSize: root.s(16); font.bold: true }
                }
            }
        }
    }
