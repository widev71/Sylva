...
Item {
    ...
    Row {
        anchors.fill: parent
        spacing: Math.round(30 * window.sf)

        Item {
            id: mainPane
            width: Math.round(1450 * window.sf)
            ...
            Behavior on tiltX { NumberAnimation { duration: 300; easing.type: Easing.OutSine } }
            Behavior on tiltY { NumberAnimation { duration: 300; easing.type: Easing.OutSine } }
            
            transform: [
                Rotation { origin.x: mainPane.width/2; origin.y: mainPane.height/2; axis.x: 1; axis.y: 0; axis.z: 0; angle: mainPane.tiltX },
                Rotation { origin.x: mainPane.width/2; origin.y: mainPane.height/2; axis.x: 0; axis.y: 1; axis.z: 0; angle: mainPane.tiltY }
            ]
            ...
            Rectangle {
                ...
                color: Qt.rgba(window.base.r, window.base.g, window.base.b, 0.90)
                ...
                // Hover Trailing Light
                Rectangle {
                    width: Math.round(300 * window.sf)
                    height: width
                    radius: width / 2
                    x: globalTiltMa.mouseX - width / 2
                    y: globalTiltMa.mouseY - height / 2
                    opacity: globalTiltMa.containsMouse ? 0.4 : 0.0
                    Behavior on opacity { NumberAnimation { duration: 400 } }
                    
                    gradient: Gradient {
                        orientation: Gradient.Horizontal
                        GradientStop { position: 0.0; color: "transparent" }
                        GradientStop { position: 0.5; color: Qt.rgba(window.activeWeatherHex.r, window.activeWeatherHex.g, window.activeWeatherHex.b, 0.5) }
