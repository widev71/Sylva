...
Item {
    ...
    Item {
        ...
        Rectangle {
            ...
            color: Qt.rgba(root.base.r, root.base.g, root.base.b, 0.45)
            radius: root.s(10)
            border.width: 1
            border.color: Qt.rgba(root.overlay0.r, root.overlay0.g, root.overlay0.b, 0.4)
            ...
            Item {
                ...
                Image {
                    ...
                    opacity: (status === Image.Ready && root.musicData.status !== "Stopped" && root.musicData.status !== "Offline") ? 0.4 : 0.0
            ...
            ColumnLayout {
                ...
                RowLayout {
                    ...
                    Item {
                        ...
                        Rectangle {
                            anchors.fill: parent
                            radius: width / 2 // Make it a perfect circle (vinyl)
                            color: root.surface1
                            ...
                            Item {
                                ...
                                RotationAnimation on rotation {
                                    ...
                                    loops: Animation.Infinite
                                    running: root.musicData.status === "Playing" && root.opacity > 0.0
                                }

                                // Vinyl Inner Ring (optional aesthetic)
                                Rectangle {
                    ...
                    ColumnLayout {
                        ...
                        // AUDIO VISUALIZER (Procedural Glow EQ)
                        Row {
                            spacing: root.s(4)
                            Layout.fillWidth: true
                            Layout.preferredHeight: root.s(40)
                            
                            // Hide if intro text hasn't slid in yet
                            opacity: root.introText
                            transform: Translate { x: root.s(30) * (1 - root.introText) }
                            ...
                            Timer {
                                running: root.musicData.status === "Playing" && root.opacity > 0.0
                                interval: 100 // Smooth update rate
                                repeat: true
                                onTriggered: parent.time += 0.15
                            }
                            
                            Repeater {
                                model: 20
                                Rectangle {
                                    width: (parent.width - root.s(4) * 19) / 20
                                    property real basePhase: index * 0.4
                                    
                                    // Generate a bouncy wave using math
                                    property real noise: Math.sin(parent.time * 2.0 + basePhase) * Math.cos(parent.time * 3.3 - basePhase)
                                    property real targetHeight: root.s(6) + Math.abs(noise) * root.s(34)
                                    
                                    height: root.musicData.status === "Playing" ? targetHeight : root.s(6)
                                    anchors.bottom: parent.bottom
                                    radius: root.s(3)
                                    
                                    color: root.musicData.status === "Playing" ? root.mauve : root.overlay0
                ...
                ColumnLayout {
                    ...
                    Item {
                        ...
                        Canvas {
                            ...
                            Timer {
                                interval: 33 // ~30fps for CPU efficiency
                                running: root.eqLightningFade < 1.0 && root.eqLightningProgress > 0.0 && root.opacity > 0.0 && root.visible
                                repeat: true
