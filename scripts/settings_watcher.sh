...
compile_settings() {
    ...
    TP_NATURAL=$(jq -r 'if has("tpNaturalScroll") then .tpNaturalScroll else true end' "$SETTINGS_FILE")
    TP_TAP=$(jq -r 'if has("tpTapToClick") then .tpTapToClick else true end' "$SETTINGS_FILE")
    TP_DWT=$(jq -r 'if has("tpDisableWhileTyping") then .tpDisableWhileTyping else true end' "$SETTINGS_FILE")
    TP_SENS=$(jq -r '.tpSensitivity // 0.0' "$SETTINGS_FILE")
    TP_SCROLL=$(jq -r '.tpScrollFactor // 1.0' "$SETTINGS_FILE")
    
    CURSOR_THEME=$(jq -r '.cursorTheme // "macOS"' "$SETTINGS_FILE")
    CURSOR_SIZE=$(jq -r '.cursorSize // 24' "$SETTINGS_FILE")
    ...
    sed -e "s|{{KB_LAYOUT}}|$LANG|g" \
        ...
        -e "s|{{TP_NATURAL}}|$TP_NATURAL|g" \
        -e "s|{{TP_TAP}}|$TP_TAP|g" \
        -e "s|{{TP_DWT}}|$TP_DWT|g" \
        -e "s|{{TP_SENS}}|$TP_SENS|g" \
        -e "s|{{TP_SCROLL}}|$TP_SCROLL|g" \
    ...
    # Apply cursor
    echo "exec-once = hyprctl setcursor \"$CURSOR_THEME\" $CURSOR_SIZE" >> "$AUTOSTART_CONF"
    ...
    if [ "$OLD_MON_HASH" != "$NEW_MON_HASH" ]; then
        ...
        # Also dynamically apply cursor immediately
